...
EN = {
    ...
    "s1_p": "Pick single files, choose a folder — subfolders included — or drag everything into the window. AutoPress reads each title from the front matter, the first heading or the file name.",
    ...
    "img_1_t": "Into the media library", "img_1_p": "Local images in the text are uploaded with their alt text and linked in the post — each one only once per site and session.",
    ...
    "c2_t": "Errors with a way out", "c2_p": "If your server swallows the <code>Authorization</code> header — common with shared hosting — AutoPress recognises it and shows the <code>.htaccess</code> line that fixes it.",
...
DE = dict(EN, **{
    ...
    "s2_p": "Beitrag oder Seite, Entwurf oder live: einmal für alle Dokumente einstellen und bei jedem einzelnen abweichen. Bevor etwas sofort veröffentlicht wird, fragt AutoPress nach — ein Mal.",
    ...
    "s3_p": "Ein Dokument nach dem anderen entsteht in WordPress, die Bilder zuerst. Ein Fortschrittsbalken zeigt, wie weit du bist — anhalten geht jederzeit.",
    ...
    "c2_t": "Fehler mit Ausweg", "c2_p": "Verschluckt dein Server den <code>Authorization</code>-Header — häufig bei Shared Hosting —, erkennt AutoPress das und zeigt die passende <code>.htaccess</code>-Zeile.",
